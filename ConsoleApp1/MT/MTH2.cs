    ...
    public static class MTH2
    {
        ...
        public static void IncrementCounter()
        {
            for (int i = 0; i < 150; i++)
            {
                ...
                //variant 3 : using interlock, which is a faster way of locking a resource
                //but has minimal feature like ++, --, +=
                int newValue = Interlocked.Increment(ref counter);
                Console.WriteLine($"Thread {Thread.CurrentThread.ManagedThreadId} incremented counter to {newValue}");
                Thread.Sleep(1);
            }
        }
